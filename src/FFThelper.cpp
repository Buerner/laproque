...
FFThelper::FFThelper( unsigned size ) :
...
void FFThelper::real2complex( float *input, fftwf_complex *output, bool normalize )
{
...
void FFThelper::complex2real( fftwf_complex *input, float *output )
{
...
unsigned FFThelper::get_spetrum_size()
{
    return _spectrum_size;
}

float FFThelper::get_norm_fact()
{
    return _norm_fact;
}

void FFThelper::set_norm_fact( float norm_fact )
{
    _norm_fact = norm_fact;
}

void FFThelper::reset_norm_fact()
{
    _norm_fact = .5f / float(_spectrum_size-1);
}
